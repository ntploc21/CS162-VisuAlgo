...
//                 0.75, 1, "Increment k, index specified has not been
//                 reached.");
//             if (k == 0)
...
//             if (playingAt == 1.0f) {
//                 AnimationFactory::DrawDirectionalArrow(start, end,
//                 true, 1.0f);
//             }
//             return srcDS;
//         });
...
//                               "(not in the actual Linked "
//                               "List).\nThe whole process is still O(1).");
//         anim9.SetAnimation([this, actualPosX](GUI::DoublyLinkedList srcDS,
//                                               float playingAt, Vector2 base)
//                                               {
//             Vector2 newPos = srcDS.GetPosition();
//             newPos.x += (actualPosX - newPos.x) * playingAt;
...
//     DLLAnimation anim2 =
//         GenerateAnimation(0.75, 1, "The head is the only vertex in this
//         List.");
//     if (nodes.size() > 1)
...
//     DLLAnimation anim5 = GenerateAnimation(
//         0.5, -1,
//         "Re-layout the Linked List for visualization (not in the actual
//         Linked " "List).\nThe whole process is still O(1).");
//     anim5.SetAnimation([this, actualPosX](GUI::DoublyLinkedList srcDS,
//                                           float playingAt, Vector2 base) {
//         Vector2 newPos = srcDS.GetPosition();
//         newPos.x += (actualPosX - newPos.x) * playingAt;
//         srcDS.SetPosition(newPos);
...
//                                   " "temp pointers advance to their next
//                                   node.");
...
//                           process " "is O(N) just to find the pre pointer.");
//     anim6.SetAnimation([this](GUI::DoublyLinkedList srcDS, float playingAt,
//                               Vector2 base) {
//         auto& nodes = srcDS.GetList();
...
//                    20 * (visualizer.GetList().size() - 1);
//     float actualPosX = (global::SCREEN_WIDTH - length) / 2;
//     DLLAnimation animEnd = GenerateAnimation(
//         0.5, -1,
//         "Re-layout the Linked List for visualization (not in the actualLinked
//         " "List).\nThe whole process is still O(1).");
//     animEnd.SetAnimation([this, actualPosX](GUI::DoublyLinkedList srcDS,
//                                             float playingAt, Vector2 base) {
//         Vector2 newPos = srcDS.GetPosition();
//         newPos.x += (actualPosX - newPos.x) * playingAt;
//         srcDS.SetPosition(newPos);
...
//         nodes[0].AnimationOnNode(true);
//         DLLAnimation anim1 = GenerateAnimation(
//             0.75, 0, "head is currently NOT NULL, proceed to the next
//             line.");
//         animController->AddAnimation(anim1);
//         nodes[0].AnimationOnNode(false);
//     }
...
//         {  // Line 2
//             DLLAnimation animLoop1 = GenerateAnimation(
//                 0.75, 2, "Increment k, index specified has not been
//                 reached.");
//             if (k == 0)
//                 animLoop1.SetActionDescription("Enter the loop.\nk is now:
//                 0");
...
// Algorithm::DoublyLinkedList::HighlightArrowFromCur(int index,
//                                                    bool drawVisualizer,
//                                                    bool reverse) {
