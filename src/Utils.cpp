...
void Utils::DrawTextBoxed(Font font, const char *text, Rectangle rec,
                          ...
                          Color tint) {
    DrawTextBoxedSelectable(font, text, rec, fontSize, spacing, wordWrap, tint,
                            0, 0, WHITE, WHITE);
}
// Draw text using font inside rectangle limits with support for text selection
void Utils::DrawTextBoxedSelectable(Font font, const char *text, Rectangle rec,
    ...
    for (int i = 0, k = 0; i < length; i++, k++) {
        ...
        // container). When wordWrap is OFF we don't need the measure state so
        // we go to the drawing state immediately and begin drawing on the next
        // line before we can get outside the container.
        if (state == MEASURE_STATE) {
            // TODO: There are multiple types of spaces in UNICODE, maybe it's a
            // good idea to add support for more Ref:
