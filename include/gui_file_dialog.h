...
GuiFileDialogState InitGuiFileDialog(const char *initPath) {
    ...
    // Init window data
    state.windowBounds =
        (Rectangle){GetScreenWidth() / 2.0F - 800 / 2.0F,
                    GetScreenHeight() / 2.0F - 600 / 2.0F, 800, 600};
    ...
    if (state->windowActive) {
        ...
        if (state->supportDrag) {
            ...
            if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
                ...
                if (CheckCollisionPointRec(
                    ...
                    state->dragMode = true;
                    state->panOffset.x =
                        mousePosition.x - state->windowBounds.x;
                    state->panOffset.y =
                        mousePosition.y - state->windowBounds.y;
                }
            }
        ...
        if (dirFilesIcon == NULL) {
            ...
            for (int i = 0; i < MAX_DIRECTORY_FILES; i++)
                dirFilesIcon[i] = (char *)RL_CALLOC(MAX_ICON_PATH_LENGTH,
                                                    1);  // Max file name length
        }
        ...
        if (GuiButton((Rectangle){state->windowBounds.x +
                                      state->windowBounds.width - 48,
                                  state->windowBounds.y + 24 + 12, 40, 24},
                      "< ..")) {
            // Move dir path one level up
            strcpy(state->dirPathText,
                   GetPrevDirectoryPath(state->dirPathText));
        ...
        state->filesListActive = GuiListViewEx(
            ...
            &state->itemFocused, &state->filesListScrollIndex,
        ...
        //|| IsKeyPressed(KEY_DPAD_A)))
        {
            ...
            if (DirectoryExists(TextFormat("%s/%s", state->dirPathText,
                                           state->fileNameText))) {
        ...
        GuiLabel(
            (Rectangle){state->windowBounds.x + 8,
                        ...
                        60, 24},
            "File name:");
        if (GuiTextBox((Rectangle){state->windowBounds.x + 90,
                                   state->windowBounds.y +
                                       state->windowBounds.height - 68,
                                   state->windowBounds.width - 202, 24},
                       state->fileNameText, 128, state->fileNameEditMode)) {
            if (*state->fileNameText) {
                // Verify if a valid filename has been introduced
                if (FileExists(TextFormat("%s/%s", state->dirPathText,
                                          state->fileNameText))) {
        ...
        state->fileTypeActive =
            GuiComboBox((Rectangle){state->windowBounds.x + 90,
                                    state->windowBounds.y +
                                        state->windowBounds.height - 24 - 12,
                                    state->windowBounds.width - 202, 24},
    ...
    for (int i = 0; i < state->dirFiles.count; i++) {
        if (IsPathFile(state->dirFiles.paths[i])) {
            ...
            if (IsFileExtension(state->dirFiles.paths[i],
                                ".png;.bmp;.tga;.gif;.jpg;.jpeg;.psd;.hdr;.qoi;"
                                ".dds;.pkm;.ktx;.pvr;.astc")) {
                strcpy(dirFilesIcon[i],
                       TextFormat("#12#%s",
                                  GetFileName(state->dirFiles.paths[i])));
                ...
                strcpy(dirFilesIcon[i],
                       TextFormat("#11#%s",
                                  GetFileName(state->dirFiles.paths[i])));
                                       ...
                                       ".txt;.info;.md;.nfo;.xml;.json;.c;.cpp;"
                                       ".cs;.lua;.py;.glsl;.vs;.fs")) {
                strcpy(dirFilesIcon[i],
                       TextFormat("#10#%s",
                                  GetFileName(state->dirFiles.paths[i])));
            } else if (IsFileExtension(state->dirFiles.paths[i],
